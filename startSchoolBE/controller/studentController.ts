  ...
  try {
    ...
    if (school?.status === "school-admin" && school) {
      ...
      if (students?.feesPaid1st === true) {
        ...
        console.log("stu", student, "student");
  ...
  try {
    ...
    if (school?.schoolName && getStudent?.schoolName) {
      if (school.verify) {
        ...
        const pass = await bcrypt.compare(password, getStudent?.password);
...
export const updateStudentProfile = async (req: any, res: Response) => {
  try {
    const { studntID } = req.params;
    const { parentEmail, password, phone, studentAddress, biography } =
      req.body;
    ...
    if (school) {
      const update = await studentModel.findByIdAndUpdate(
        studntID,
        { parentEmail, password, phone, studentAddress, biography },
        { new: true }
      );
